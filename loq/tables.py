...
class FavTable(SingleTableMixin,tables.Table):
    mirna = tables.LinkColumn('IntervalDetailView',args=[A('pk')],accessor="target.mirName", verbose_name="miRNA")
    class Meta:
        model = Favorite
        attrs = {"class":"paleblue"}
        fields =('timestamp',)
...
class CommentTable(SingleTableMixin,tables.Table):
    mirna = tables.LinkColumn('IntervalDetailView',args=[A('pk')],accessor="content_object.mirName", verbose_name="miRNA")
    class Meta:
        model = Comment
        attrs = {"class":"paleblue"}
